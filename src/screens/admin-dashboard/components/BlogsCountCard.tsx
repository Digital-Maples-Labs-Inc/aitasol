...
export default function BlogsCountCard() {
  const theme = useTheme();
  const [publishedCount, setPublishedCount] = React.useState<number>(0);
  const [unpublishedCount, setUnpublishedCount] = React.useState<number>(0);
  ...
  React.useEffect(() => {
    const loadBlogs = async () => {
      try {
        ...
        const unpublished = blogs.filter(b => b.status !== 'published').length;
  ...
  const trend = publishedCount > unpublishedCount ? 'up' : 'neutral';
          ...
          <Stack sx={{ justifyContent: 'space-between' }}>
            ...
            <Stack direction="row" spacing={1} sx={{ mt: 1 }}>
              <Chip 
                ...
                onClick={() => {
                  if (typeof window !== 'undefined') {
                    window.location.href = '/admin/blogs?status=published';
                  }
                }}
                sx={{ cursor: 'pointer' }}
              />
              <Chip 
                size="small" 
                variant="outlined"
                label={`${unpublishedCount} draft`}
                color="default"
                onClick={() => {
                  if (typeof window !== 'undefined') {
                    window.location.href = '/admin/blogs?status=draft';
                  }
                }}
                sx={{ cursor: 'pointer' }}
              />
            ...
            <SparkLineChart
              ...
              sx={{
                [`& .${areaElementClasses.root}`]: {
                  fill: `url(#area-gradient-blogs)`,
                },
              }}
            >
              <AreaGradient color={chartColor} id="area-gradient-blogs" />
            </SparkLineChart>
          </Box>
        </Stack>
      </CardContent>
